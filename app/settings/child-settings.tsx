...
export default function ChildSettings({ childInfo }: ChildSettingsProps) {
  ...
  const [selectedInterests, setSelectedInterests] = useState<string[]>(childInfo.interests);
  const [favoriteColor, setFavoriteColor] = useState(childInfo.favoriteColor);
  const [region, setRegion] = useState(childInfo.region);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem("childInfo", JSON.stringify(updatedChildInfo));
      setIsSuccess(true);
    } catch (err) {
      setError("保存に失敗しました");
    } finally {
